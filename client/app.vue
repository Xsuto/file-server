...
const formattedServerFiles = $computed(() => serverFiles.map(it =>
  ({ ...it, createdAt: `${it.createdAt.toString().split('T')[0]} ${it.createdAt.toString().split('T')[1].split('.')[0]}` }),
))
...
<template>
  <div>
    ...
    <p v-for="file in formattedServerFiles" :key="file" @click="handleClick(file)">
